...
public interface RequestRepository extends CrudRepository<RequestDB, Long> {
    ...
    @Query(value="select a.name, concat(\"/sec/my-request-detail/\", a.id), a.sign_id, a.name as sign_name, a.media_type from requests a where a.name like concat(:name,'%') and a.user_id = :userId and a.sign_id is null union select b.name, concat(\"/sec/other-request-detail/\", b.id), b.sign_id, b.name as sign_name, b.media_type  from requests b where b.name like concat(:name,'%') and b.user_id != :userId and b.sign_id is null", nativeQuery = true)
    List<Object[]> findRequestsByNameWithNoAssociateSign(@Param("name") String name,@Param("userId") long userId);

    @Query(value="select a.name, concat(\"/sec/my-request-detail/\", a.id), a.sign_id, c.name as sign_name, a.media_type  from requests a join signs c on c.id = a.sign_id and c.name != a.name and a.name like concat(:name,'%') and a.user_id = :userId and a.sign_id is not null union select b.name, concat(\"/sec/other-request-detail/\", b.id), b.sign_id, d.name as sign_name, b.media_type from requests b join signs d on d.id = b.sign_id and d.name != b.name and b.name like concat(:name,'%') and b.user_id != :userId and b.sign_id is not null", nativeQuery = true)
