...
public interface SignRepository extends CrudRepository<SignDB, Long> {
    ...
    @Query(value="from VideoDB a inner join a.sign b WHERE a.id = b.lastVideoId order by name asc")
    List<Object[]> findSignsAlphabeticalOrderAscForSignsView();

    @Query(value="from VideoDB a inner join a.sign b WHERE a.id = b.lastVideoId order by name desc")
    ...
    @Query(value="select * from signes", nativeQuery = true)
    ...
    @Query(value="from VideoDB a inner join a.sign b WHERE b.id = a.id and a.id = :signId order by a.createDate desc")
